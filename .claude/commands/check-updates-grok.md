Query Grok 4.1 via OpenRouter to check for new attack waves or updated data for the Iran-Israel conflict dataset.
...
3. Call Grok 4.1 Fast via OpenRouter using the `OPENROUTER_API_KEY` from `.env`. Use this Python pattern:
...
5. **Critically evaluate the response.** Cross-reference Grok's claims against existing data. If Grok returns data that contradicts existing curated records (different codenames, timestamps, or munitions for waves already in the DB), flag these discrepancies and DO NOT overwrite existing data. Only incorporate data that:
